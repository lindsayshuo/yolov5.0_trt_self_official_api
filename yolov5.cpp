...
//    std::pair<std::map<std::string, Weights>, std::vector<std::string>> WTS = loadWeights(wts_name);
//    std::map<std::string, Weights> weightMap  = WTS.first;
//    std::vector<std::string> classNames = WTS.second;
...
//		std::cout << engine->TClasses.size() << std::endl;
//        engine->TClasses.push_back(name.size());
//    }
...
void video(){
	Mat frame;
	VideoCapture cap("../guowang.avi");
	int key = 10;
	for(;;){
		cap.read(frame);
		if(frame.empty()) break;
		imshow("a", frame);
		if(waitKey(key) == 27) break;
	}
}
